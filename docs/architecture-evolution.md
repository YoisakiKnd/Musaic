...
> **范围界定（重要）**：本设计只把**收藏与历史**改为按作品归一。
> **歌单明确不在改造范围**——见 §2.4 的范围说明。
...
> #### 范围说明：歌单**不在**改造范围（产品决策）
>
> 用户明确要求保留「同一个本地歌单里可以存放来自不同渠道的同一首歌」这一特性。
> 这是 Musaic「多渠道聚合」的核心价值：**选择权交给用户**——同一首歌，
> 网易云可能音质更好、QQ 可能有独家版权、酷狗可能免费，用户需要自己挑。
>
> 因此三者的去重语义**刻意不同**：
>
> | 数据 | 语义 | 去重键 | 理由 |
> |---|---|---|---|
> | **歌单** | 「我**挑选**的播放清单」 | `track.key`（渠道相关） | 用户要自己选音源，多渠道版本必须共存 |
> | **收藏** | 「我**喜欢**这首歌」 | `WorkId`（作品归一） | 喜欢的是歌，不是某个渠道的某条记录 |
> | **历史** | 「我**听过**这首歌」 | `WorkId`（作品归一） | 同理，听的是歌 |
>
> 歌单现状**已经正确**（`library_repository.dart` 按 `track.key` 去重），
> 本设计**不需要改动它**。此契约由
> `test/features/library/multi_source_playlist_test.dart` 固化——
> 若有人「顺手」把歌单去重也改成作品归一，该测试会立刻失败。
...
- 移动/重命名文件 → **收藏与歌单都保持有效**（路径变化不影响 id）。
- 内容相同但路径不同 → 视为同一首（符合用户直觉）。
- 迁移：旧数据 id 是路径，需在 schema 迁移中按「路径仍存在 → 重算 id 并改键；
  路径不存在 → 保留原键并标记 `orphan`」（**不删**，用户可能只是外接盘没插）。
- **注意**：本地 id 改造会让**同一首歌在不同路径**归为同一 key，
  从而在歌单里去重。这是**期望行为**（用户眼里就是一个文件），
  与「不同渠道版本共存」不冲突——后者跨渠道，前者同渠道。
...
| **S4** | §2.4 收藏/历史迁移到 workId（**歌单不动**） | 迁移后旧收藏完整保留；跨渠道收藏点亮（集成验证）；歌单多渠道共存契约测试仍全绿 |
...
| **误把歌单也归一化，吃掉用户挑选的多渠道版本** | 歌单去重键保持 `track.key`；`multi_source_playlist_test.dart` 固化契约（已用变异验证：改成归一化会让 5 条测试变红） |
