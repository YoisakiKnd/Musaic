...
abstract final class TtmlParser {
  static LyricBundle parse(String source) {
    ...
    for (final node in doc.descendantElements.where(
      (e) => e.name.local == 'p',
    )) {
      ...
      final end =
          _attr(node, 'end') != null ? parseClock(_attr(node, 'end')!) : null;
      ...
      for (final child in node.childElements) {
        ...
        if (spanBegin != null) {
          ...
          if (wStart != null) {
            final wEnd =
                _attr(child, 'end') != null
                    ? parseClock(_attr(child, 'end')!)
                    : null;
            words.add(LyricWord(text: text, start: wStart, end: wEnd));
          }
        }
      }
      if (buffer.isEmpty) {
    ...
    for (final meta in doc.descendantElements.where(
      (e) => e.name.local == 'metadata',
    )) {
    ...
    return LyricBundle(
      lines: lines,
      granularity: hasWords ? LyricGranularity.word : LyricGranularity.line,
      metadata: metadata,
    );
  }
  ...
  static Duration? parseClock(String value) {
    ...
    if (ms != null) {
      return Duration(
        microseconds: ((double.parse(ms.group(1)!)) * 1000).round(),
      );
    }
    // 秒形式：12.5s
    final s = RegExp(r'^(\d+(?:\.\d+)?)s$').firstMatch(v);
    if (s != null) {
      return Duration(microseconds: (double.parse(s.group(1)!) * 1e6).round());
    }
    // 冒号时钟：s | m:s | h:m:s（最后一段允许小数）
    final parts = v.split(':');
    final clockPattern = RegExp(r'^\d{1,3}(?:\.\d{1,3})?$');
    if (parts.length <= 3 && parts.every(clockPattern.hasMatch)) {
      ...
      return Duration(microseconds: (secondsTotal * 1e6).round());
    }
