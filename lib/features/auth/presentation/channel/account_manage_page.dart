              ...
              onPressed: () async {
                ...
                if (confirmed == true) {
                  ...
                  ScaffoldMessenger.of(
                    context,
                  ).showSnackBar(const SnackBar(content: Text('账号数据已清除')));
                }
              },
...
class _ChannelEntry extends ConsumerWidget {
                        ...
                        final confirmed = await showDialog<bool>(
                          ...
                          builder:
                              (dialogContext) => AlertDialog(
                                ...
                                content: const Text('将删除本机保存的登录凭据。'),
                                actions: [
                                  TextButton(
                                    onPressed:
                                        () => Navigator.of(
  ...
  Future<void> _refreshAccount(
    ...
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('刷新失败，请检查网络或重新登录')));
    }
  }
  ...
  void _showComingSoon(BuildContext context, String channel, String feature) {
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(SnackBar(content: Text('$channel $feature 开发中，敬请期待')));
  }
}
